...
function renderBuilderGeometryOnly(){let canvas=$('#builderCanvas');if(!canvas||!P)return;canvas.style.setProperty('--grid-cell',CELL+'px');canvas.style.setProperty('width',(GRID_COLS*CELL)+'px','important');canvas.style.setProperty('height',(GRID_ROWS*CELL)+'px','important');canvas.style.setProperty('min-width',(GRID_COLS*CELL)+'px','important');canvas.style.setProperty('min-height',(GRID_ROWS*CELL)+'px','important');canvas.querySelectorAll('.builder-item').forEach(el=>{let t=P.tables.find(x=>x.id===el.dataset.bid);if(t)gridStyle(el,gridRect(t))});canvas.querySelectorAll('.hall-object').forEach(el=>{let o=P.hall_objects?.find(x=>x.id===el.dataset.oid);if(o)gridStyle(el,gridRect(o))})}
