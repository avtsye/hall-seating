...
function render(){if(!P)return;P.hall.seating_type??='round_tables';P.hall_objects??=[];P.settings??={};P.settings.seat_preference??='front_center';P.settings.front_weight??=1;P.settings.center_weight??=.35;P.settings.show_quality??=false;P.settings.assignment_zoom??=1;P.settings.grid_cols??=26;P.settings.grid_rows??=16;P.tables.forEach(t=>{t.kind??='table';t.shape??='round'});$('#projectTitle').textContent=P.name;$('#hallName').value=P.hall.name;$('#stageLabel').value=P.hall.stage_label;$('#seatingType').value=P.hall.seating_type;renderBuilder();renderHall('#hallCanvas',true);renderHall('#assignCanvas',false);renderAssignmentPro();renderDisplayMap();renderGroups();renderPeople();renderRules();renderWaiting();renderVersions();renderProjects();renderStats()}
...
function mergedObjectPath(objects){
 const cells=new Set();objects.forEach(o=>cellSquares(gridRect(o)).forEach(p=>cells.add(p.x+','+p.y)));
 const has=(x,y)=>cells.has(x+','+y),edges=[];
...
function renderMergedHallObjects(container){
 container.querySelectorAll('.merged-hall-objects').forEach(n=>n.remove());let objs=P?.hall_objects||[];if(!objs.length)return;
 let svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.classList.add('merged-hall-objects');svg.setAttribute('width',GRID_COLS*CELL);svg.setAttribute('height',GRID_ROWS*CELL);svg.setAttribute('viewBox','0 0 '+(GRID_COLS*CELL)+' '+(GRID_ROWS*CELL));
 ['stage','aisle','zone'].forEach(kind=>{let same=objs.filter(o=>(o.kind||'zone')===kind);if(!same.length)return;let path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',mergedObjectPath(same));path.setAttribute('class','merged-object merged-'+kind);svg.appendChild(path)});container.prepend(svg)
}
...
function renderHall(sel,editable){let h=$(sel);if(!h)return;h.innerHTML='';commonMapSetup(h);h.classList.toggle('edit',editable&&$('#editMode').checked);let by={};P.people.forEach(x=>{if(x.table_id)(by[x.table_id]??=[]).push(x)});
 P.hall_objects??=[];renderMergedHallObjects(h);
...
async function autoAssign(){let btn=$('#autoAssign2')||$('#autoAssign');if(btn){btn.disabled=true;btn.dataset.oldText=btn.textContent;btn.textContent='משבץ...'}try{let d=await api('/api/assign',{method:'POST'});S=d.state;P=d.project;V=d.violations||[];render();let seated=P.people.filter(x=>x.table_id).length;toast(d.unseated?.length?'השיבוץ הסתיים: '+seated+' שובצו, '+d.unseated.length+' לא שובצו':'השיבוץ החכם הושלם: '+seated+' שובצו')}catch(err){toast('השיבוץ נכשל: '+err.message)}finally{if(btn){btn.disabled=false;btn.textContent=btn.dataset.oldText||'✨ שיבוץ חכם'}}}
$('#autoAssign')?.addEventListener('click',autoAssign);$('#autoAssign2')?.addEventListener('click',autoAssign);
...
function applyGridDisplaySize(v,save=true){CELL=clampGridPx(v);if(save)localStorage.setItem('hallBuilderGridPx',CELL);let canvas=$('#builderCanvas');if(canvas){canvas.style.setProperty('--grid-cell',CELL+'px');canvas.style.setProperty('width',(GRID_COLS*CELL)+'px','important');canvas.style.setProperty('height',(GRID_ROWS*CELL)+'px','important');canvas.style.setProperty('min-width',(GRID_COLS*CELL)+'px','important');canvas.style.setProperty('min-height',(GRID_ROWS*CELL)+'px','important')}let input=$('#gridCellSize'),out=$('#gridCellSizeValue');if(input)input.value=CELL;if(out)out.value=out.textContent=CELL+'px';$$('[data-grid-px]').forEach(b=>b.classList.toggle('active',+b.dataset.gridPx===CELL));if(P)renderBuilderGeometryOnly()}
...
function assignmentTooltip(t,occ){
...
function renderDisplayMap(){
 ...
 P.tables.forEach((t,index)=>{let occ=P.people.filter(x=>x.table_id===t.id).sort((a,b)=>(a.seat??0)-(b.seat??0)),d=document.createElement('div');d.className='display-seat '+seatIconClass(t);commonMapStyle(d,gridRect(t));let number=esc(t.name||('מקום '+(index+1))),primary=occ[0];d.innerHTML='<div class="display-number">'+number+'</div><div class="display-person">'+(primary?esc(primary.name):'<span class="display-empty">פנוי</span>')+'</div>';d.tabIndex=0;d.setAttribute('role','button');d.setAttribute('aria-label',(t.name||'מקום')+(occ.length?' - '+occ.map(x=>x.name).join(', '):' - פנוי'));d.onmouseenter=e=>showDisplayTooltip(t,occ,e.currentTarget);d.onmouseleave=hideDisplayTooltip;d.onfocus=e=>showDisplayTooltip(t,occ,e.currentTarget);d.onblur=hideDisplayTooltip;d.onclick=()=>{hideDisplayTooltip();openDisplaySeat(t.id)};d.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();hideDisplayTooltip();openDisplaySeat(t.id)}};h.appendChild(d)});setDisplayZoom(displayZoom)
}

function displaySeatTooltip(t,occ){let rows=occ.length?occ.map(x=>'<div class="display-tip-person"><b>'+esc(x.name)+'</b>'+(group(x.group_id)?'<span>'+esc(group(x.group_id).name)+'</span>':'')+(x.note?'<small>'+esc(x.note)+'</small>':'')+'</div>').join(''):'<div class="empty-tip">המקום פנוי</div>';return '<strong>'+esc(t.name||'מקום')+'</strong>'+rows+'<small class="tip-action">לחץ לפרטים ועריכה</small>'}
function displayTooltipEl(){let el=$('#displayFloatingTooltip');if(el)return el;el=document.createElement('div');el.id='displayFloatingTooltip';el.className='display-floating-tooltip';document.body.appendChild(el);return el}
function showDisplayTooltip(t,occ,anchor){let el=displayTooltipEl();el.innerHTML=displaySeatTooltip(t,occ);el.hidden=false;let r=anchor.getBoundingClientRect(),w=el.offsetWidth,h=el.offsetHeight,x=Math.max(8,Math.min(innerWidth-w-8,r.left+r.width/2-w/2)),y=r.top-h-9;if(y<8)y=Math.min(innerHeight-h-8,r.bottom+9);el.style.left=x+'px';el.style.top=y+'px'}
function hideDisplayTooltip(){let el=$('#displayFloatingTooltip');if(el)el.hidden=true}
let displaySeatId=null;
function openDisplaySeat(id){let t=table(id);if(!t)return;displaySeatId=id;let occ=P.people.filter(x=>x.table_id===id).sort((a,b)=>(a.seat??0)-(b.seat??0));$('#displaySeatTitle').textContent=t.name||'פרטי מקום';$('#displaySeatInfo').innerHTML='<div><b>סוג:</b> '+esc(t.kind==='seat'?'כיסא':'שולחן')+'</div><div><b>קיבולת:</b> '+t.capacity+'</div>'+(t.zone?'<div><b>אזור:</b> '+esc(t.zone)+'</div>':'')+((t.tags||[]).length?'<div><b>תגיות:</b> '+esc(t.tags.join(', '))+'</div>':'');$('#displaySeatOccupants').innerHTML=occ.length?'<h3>יושבים במקום</h3>'+occ.map(x=>'<button type="button" class="display-person-card" data-person="'+x.id+'"><b>'+esc(x.name)+'</b>'+(group(x.group_id)?'<span>'+esc(group(x.group_id).name)+'</span>':'')+(x.note?'<small>'+esc(x.note)+'</small>':'')+'<em>עריכת פרטים</em></button>').join(''):'<div class="display-empty-card">המקום פנוי — ניתן לשבץ אליו אדם דרך עריכת המקום.</div>';$('#displaySeatOccupants [data-person]').forEach(b=>b.onclick=()=>{let dlg=$('#displaySeatDlg');if(dlg.open)dlg.close();requestAnimationFrame(()=>openPersonPrefs(b.dataset.person))});let dlg=$('#displaySeatDlg');if(dlg.open)dlg.close();dlg.showModal()}
$('#editDisplaySeat')?.addEventListener('click',()=>{if(!displaySeatId)return;let id=displaySeatId,dlg=$('#displaySeatDlg');if(dlg.open)dlg.close();requestAnimationFrame(()=>openTable(id))});
